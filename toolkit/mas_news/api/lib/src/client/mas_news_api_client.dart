...
import 'package:mas_news_api/client.dart';
import 'package:http/http.dart' as http;

/// {@template mas_news_api_malformed_response}
/// An exception thrown when there is a problem decoded the response body.
/// {@endtemplate}
class MasNewsApiMalformedResponse implements Exception {
  /// {@macro mas_news_api_malformed_response}
  const MasNewsApiMalformedResponse({required this.error});

  /// The associated error.
  final Object error;
}

/// {@template mas_news_api_request_failure}
/// An exception thrown when an http request failure occurs.
/// {@endtemplate}
class MasNewsApiRequestFailure implements Exception {
  /// {@macro mas_news_api_request_failure}
  const MasNewsApiRequestFailure({
...
/// {@template mas_news_api_client}
/// A Dart API client for the Mas News API.
/// {@endtemplate}
class MasNewsApiClient {
  /// Create an instance of [MasNewsApiClient] that integrates
  /// with the remote API.
  ///
  /// {@macro mas_news_api_client}
  MasNewsApiClient({
  ...
  Future<ArticleResponse> getArticle({
    ...
    if (response.statusCode != HttpStatus.ok) {
      throw MasNewsApiRequestFailure(
        body: body,
  ...
  Map<String, dynamic> json() {
    try {
      ...
      Error.throwWithStackTrace(
        MasNewsApiMalformedResponse(error: error),
        stackTrace,
      );
    }
  }
